...
sell_types = {"piece": {"russian": "шт.", "english": "pc."}}
...
async def gen_items_result(server: Server, search: str, filter_type: str):
    ...
    for item in items:
        ...
        if shop.working and shop.subscribe_paid and not item.hide and item.amount > 0:
            curr_item = await get_mc_item(item.currency)
            result.append(
                types.InlineQueryResultArticle(item.id, mc_item.ru_name,
                                               types.InputTextMessageContent(f"Я хочу заказать {item.id}"),
                                               description=f"{shop.shop_name} - {item.price} {curr_item.ru_name.lower()}\nПокупка от {item.min_amount} {sell_types[item.sell_type]['russian']}",
...
async def buy_callback(call: types.CallbackQuery):
    ...
    cart_items_count = len(user.cart[server.id]) if server.id in user.cart else 0
    if cart_items_count > 0:
        markup.add(
            types.InlineKeyboardButton(f"Корзина ({cart_items_count})",
...
async def item_info(message: types.Message):
    item_id = message.text.split(" ")
    item_id = int(item_id[len(item_id) - 1])
...
async def select_server(message: types.Message):
    ...
    markup.add(types.InlineKeyboardButton("Открыть корзину", callback_data=f"open_cart;{server.id}"))
